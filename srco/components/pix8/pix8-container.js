...
import '/node_modules/dragula/dist/dragula.min.js';

if(!$('link[href$="dragula.min.css"]').length)
	$('<link>', {href: '/node_modules/dragula/dist/dragula.min.css', rel:"stylesheet"}).appendTo('body');
...
class element extends LitElement{
  ...
  static get styles(){
	...
	connectedCallback(){
		...
		this.carousels = Array.prototype.slice.call(this.children);

		//if(!this.drake) this.init_drake();
	}

	init_drake(){
		var drake = this.drake = dragula(this.carousels,{
		...
		drake.on('drag', (el, source) => {
			el.classList.add('dragging_el');
		}).on('dragend', (el) => {
			$('.dragging_in').removeClass('dragging_in');
			$('.dragging_el').removeClass('dragging_el');
		}).on('over', (el, container, source) => {
			$('.dragging_in').removeClass('dragging_in');
			container.classList.add('dragging_in');
		});
	}
	...
	init_resize(){
		...
		var lastCarousel, carouselHeight, left, clientX;
		interact(this.select('#nav')).draggable({
			onstart: ev => {
				lastCarousel = this.querySelector('pix8-carousel:last-of-type');
			},
			onmove: ev => {
				let newHeight = lastCarousel.offsetHeight + ev.dy;

				let scroll = lastCarousel.scroll() + ev.clientX,
					height = lastCarousel.offsetHeight;
				...
				lastCarousel.style.height = newHeight;
				lastCarousel.scroll((scroll * newHeight / height) - ev.clientX);
			},
			onend: ev => {
				console.log(ev);
			}
		...
		$tag.drag("start", function(ev, dd){
			...
			carousel.scrollLeft = dd.left + newL - dd.left - dd.clientX;
	...
	selectAll(qs){
		return Array.prototype.slice.call(
			document.shadowRoot.querySelectorAll(qs)
		);
	}
