...
WS.prototype = {
	...
	download: async function(ref){
		if(this.dlp) await this.dlp.catch((err) => {
			console.error(err);
	    });
		...
		return this.dlp = new Promise(async (resolve, reject) => {
		...
		var task = function(){
			...
			ws.send({
				...
				if(r.name){
					...
					$('#progress-tasks').text((ws.tasks.length+1));
