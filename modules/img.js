var img = window.Img = {
	...
	build: function(inf){
		...
		*/
		...
		var img = new Image;
		img.src = Cfg.files + inf.file;//+((!inf.w || (inf.w > img.w && inf.h > img.h))?"?thumb":'');
		img.onload = function(){
			//$el.append(img);
		}
					...
					ws.upload(f, function(file){
						if(file){
							...
							ws.send({
								...
								$('#gallery-upload').blink(r.item?'green':'red');
							});

							img.$cont.append(Img.build(item));
						}
					}, {
						name: f.fileName || f.name,
						width: image.width,
...
$(function(){
	Tree.types.gallery = function(item){
		ws.send({
			cmd: 'load',
			sort: {pos: 1},
			filter: {
			...
			Site.openApp('gallery');

			$('#gallery-images').data('tid', item.id).img(r.items);

			var adm = (Acc.user && item.owner == Acc.user.id);
			$('#gallery .admin').showIf(adm);

			Tree.activate(item.id);

			if(adm)
				$('#gallery-images > .thumb').draggable(img.move);
		});
	};
